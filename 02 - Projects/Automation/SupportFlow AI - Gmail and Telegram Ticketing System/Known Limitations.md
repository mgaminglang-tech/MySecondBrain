...
| LIM-018 | Gemini free-tier availability, exact model, rate limits, and n8n structured-output compatibility may change | Model cannot yet be finalized | Resolve privacy acceptance, then verify the credential-grounded model list before selection | Mervin | deferred |
| LIM-019 | Saved Phase 1 output uses schema `0.1`, legacy source fields, and JSON-array fingerprint composition | It does not yet match the newly approved `0.1.0` integration contract | Obtain separate approval for an inactive credential-free migration and rerun fixtures 001–006 | Mervin | blocking credentials |
| LIM-020 | Gemini free-tier content may be used to improve provider products | Current free-only and privacy preferences are in tension even with sanitized fixtures | Mervin must explicitly accept sanitized-fixture-only use or approve another boundary | Mervin | blocking Gemini credential |
